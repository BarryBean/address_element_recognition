...
def read_instance_with_gaz(num_layer, input_file, gaz, word_alphabet, biword_alphabet, biword_count, char_alphabet,
                           gaz_alphabet, gaz_count, gaz_split, label_alphabet, bound_alphabet, type_alphabet,
                           number_normalized,
                           max_sent_length,
    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            bound_Ids.append(bound_alphabet.get_index(bound))
            type_Ids.append(type_alphabet.get_index(type))
            char_list = []
            char_Id = []
            for char in word:
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words) > 0):
                ...
                instence_texts.append([words, biwords, chars, gazs, labels, bound, types])
                instence_Ids.append(
                    [word_Ids, biword_Ids, char_Ids, gaz_Ids, label_Ids, bound_Ids, type_Ids, gazs, gazs_count,
                     gaz_char_Id,
                     layergazmasks,
