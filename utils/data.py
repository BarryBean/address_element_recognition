...
class Data:
    def __init__(self):
        ...
        self.bound_alphabet = Alphabet('bound', True)
        self.type_alphabet = Alphabet('type', True)
        ...
        self.bound_alphabet_size = 0
        self.type_alphabet_size = 0
        ### hyperparameters
    ...
    def build_alphabet(self, input_file):
        ...
        for idx in range(len(in_lines)):
            ...
            if len(line) > 2:
                ...
                bound = pairs[2]
                self.bound_alphabet.add(bound)

                type = pairs[-1]
                self.type_alphabet.add(type)
        ...
        self.label_alphabet_size = self.label_alphabet.size()
        self.bound_alphabet_size = self.bound_alphabet.size()
        self.type_alphabet_size = self.type_alphabet.size()
        self.tagScheme = "BIOES"
    ...
    def fix_alphabet(self):
        ...
        self.bound_alphabet.close()
        self.type_alphabet.close()
    ...
    def generate_instance_with_gaz(self, input_file, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance_with_gaz(self.HP_num_layer, input_file, self.gaz,
                                                                      self.word_alphabet, self.biword_alphabet,
                                                                      self.biword_count, self.char_alphabet,
                                                                      self.gaz_alphabet, self.gaz_count, self.gaz_split,
                                                                      self.label_alphabet, self.bound_alphabet,
                                                                      self.type_alphabet,
                                                                      self.number_normalized,
                                                                      self.MAX_SENTENCE_LENGTH)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance_with_gaz(self.HP_num_layer, input_file, self.gaz,
                                                                  self.word_alphabet, self.biword_alphabet,
                                                                  self.biword_count, self.char_alphabet,
                                                                  self.gaz_alphabet, self.gaz_count, self.gaz_split,
                                                                  self.label_alphabet, self.bound_alphabet,
            ...
            self.test_texts, self.test_Ids = read_instance_with_gaz(self.HP_num_layer, input_file, self.gaz,
                                                                    self.word_alphabet, self.biword_alphabet,
                                                                    self.biword_count, self.char_alphabet,
                                                                    self.gaz_alphabet, self.gaz_count, self.gaz_split,
                                                                    self.label_alphabet, self.bound_alphabet,
            ...
            self.raw_texts, self.raw_Ids = read_instance_with_gaz(self.HP_num_layer, input_file, self.gaz,
                                                                  self.word_alphabet, self.biword_alphabet,
                                                                  self.biword_count, self.char_alphabet,
                                                                  self.gaz_alphabet, self.gaz_count, self.gaz_split,
                                                                  self.label_alphabet, self.bound_alphabet,
