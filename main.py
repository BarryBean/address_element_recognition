...
def evaluate(data, model, name):
    ...
    trans_matrix = np.zeros((data.bound_alphabet_size + 2, data.label_alphabet_size + 2), dtype=float)
    trans_matrix[0, 0] = 1  # pad to pad
    trans_matrix[1, 1] = 1  # O to O
    trans_matrix[2, 2] = 1 / 17  # B to B-MISC
    trans_matrix[2, 5] = 1 / 17  # B to B-PER
    trans_matrix[2, 8] = 1 / 17  # B to B-ORG
    trans_matrix[2, 11] = 1 / 17  # B to B-LOC
    trans_matrix[2, 14] = 1 / 17
    trans_matrix[2, 17] = 1 / 17
    trans_matrix[2, 20] = 1 / 17
    trans_matrix[2, 23] = 1 / 17
    trans_matrix[2, 26] = 1 / 17
    trans_matrix[2, 29] = 1 / 17
    trans_matrix[2, 32] = 1 / 17
    trans_matrix[2, 35] = 1 / 17
    trans_matrix[2, 38] = 1 / 17
    trans_matrix[2, 41] = 1 / 17
    trans_matrix[2, 44] = 1 / 17
    trans_matrix[2, 47] = 1 / 17
    trans_matrix[2, 50] = 1 / 17
    trans_matrix[3, 3] = 1 / 17  # I to I
    trans_matrix[3, 6] = 1 / 17
    trans_matrix[3, 9] = 1 / 17
    trans_matrix[3, 12] = 1 / 17
    trans_matrix[3, 15] = 1 / 17
    trans_matrix[3, 18] = 1 / 17
    trans_matrix[3, 21] = 1 / 17
    trans_matrix[3, 24] = 1 / 17
    trans_matrix[3, 27] = 1 / 17
    trans_matrix[3, 30] = 1 / 17
    trans_matrix[3, 33] = 1 / 17
    trans_matrix[3, 36] = 1 / 17
    trans_matrix[3, 39] = 1 / 17
    trans_matrix[3, 42] = 1 / 17
    trans_matrix[3, 45] = 1 / 17
    trans_matrix[3, 48] = 1 / 17
    trans_matrix[3, 51] = 1 / 17
    trans_matrix[4, 4] = 1 / 17  # E to X
    trans_matrix[4, 7] = 1 / 17
    trans_matrix[4, 10] = 1 / 17
    trans_matrix[4, 13] = 1 / 17
    trans_matrix[4, 16] = 1 / 17
    trans_matrix[4, 19] = 1 / 17
    trans_matrix[4, 22] = 1 / 17
    trans_matrix[4, 25] = 1 / 17
    trans_matrix[4, 28] = 1 / 17
    trans_matrix[4, 31] = 1 / 17
    trans_matrix[4, 34] = 1 / 17
    trans_matrix[4, 37] = 1 / 17
    trans_matrix[4, 40] = 1 / 17
    trans_matrix[4, 43] = 1 / 17
    trans_matrix[4, 46] = 1 / 17
    trans_matrix[4, 49] = 1 / 17
    trans_matrix[4, 52] = 1 / 17
    trans_matrix[5, 53] = 0.2  # S to X
    trans_matrix[5, 54] = 0.2
    trans_matrix[5, 55] = 0.2
    trans_matrix[5, 56] = 0.2
    trans_matrix[5, 57] = 0.2
    trans_matrix[6, 58] = 1
    trans_matrix[7, 59] = 1
    ...
    for batch_id in range(total_batch):
        with torch.no_grad():
            ...
            if not instance:
                continue
            gaz_list, batch_word, batch_biword, batch_wordlen, batch_label, batch_bound, batch_type, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask = batchify_with_label(
            ...
            tag_seq, gaz_match = model(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,
                                       gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask, trans_matrix)
    ...
    # submit_test = open(os.path.join('addr_parsing_runid_lex.txt'), 'w', encoding='utf8')
    # final_test = open(r'./data/Address/mytest.txt', 'r', encoding='utf8')
    # for idx, (test_label_list_i, line) in tqdm(enumerate(zip(pred_results, final_test.readlines())),
...
def batchify_with_label(input_batch_list, gpu, num_layer, volatile_flag=False):
    ...
    bounds = [sent[5] for sent in input_batch_list]
    types = [sent[6] for sent in input_batch_list]
    layer_gazs = [sent[7] for sent in input_batch_list]
    gaz_count = [sent[8] for sent in input_batch_list]
    gaz_chars = [sent[9] for sent in input_batch_list]
    gaz_mask = [sent[10] for sent in input_batch_list]
    gazchar_mask = [sent[11] for sent in input_batch_list]
    ### bert tokens
    bert_ids = [sent[12] for sent in input_batch_list]
    ...
    bound_seq_tensor = autograd.Variable(torch.zeros((batch_size, max_seq_len))).long()
    type_seq_tensor = autograd.Variable(torch.zeros((batch_size, max_seq_len))).long()
    ...
    for b, (seq, bert_id, biseq, label, bound, type, seqlen, layergaz, gazmask, gazcount, gazchar, gazchar_mask, gaznum,
            gazlen) in enumerate(
        zip(words, bert_ids, biwords, labels, bounds, types, word_seq_lengths, layer_gazs, gaz_mask, gaz_count,
            gaz_chars,
        ...
        bound_seq_tensor[b, :seqlen] = torch.LongTensor(bound)
        type_seq_tensor[b, :seqlen] = torch.LongTensor(type)
    ...
    if gpu:
        ...
        bound_seq_tensor = bound_seq_tensor.cuda()
        type_seq_tensor = type_seq_tensor.cuda()
    ...
    return gazs, word_seq_tensor, biword_seq_tensor, word_seq_lengths, label_seq_tensor, bound_seq_tensor, type_seq_tensor, layer_gaz_tensor, gaz_count_tensor, gaz_chars_tensor, gaz_mask_tensor, gazchar_mask_tensor, mask, bert_seq_tensor, bert_mask
...
def train(data, save_model_dir, seg=True):
    ...
    # data.show_data_summary()
    # model = SeqModel(data)
    # model.load_state_dict(torch.load(save_model_dir, map_location='cpu'))
    ...
    trans_matrix[0, 0] = 1  # pad to pad
    trans_matrix[1, 1] = 1  # O to O
    # trans_matrix[2, 2] = 1 / 17
    # trans_matrix[2, 4] = 1 / 17
    # trans_matrix[2, 6] = 1 / 17
    # trans_matrix[2, 8] = 1 / 17
    # trans_matrix[2, 10] = 1 / 17
    # trans_matrix[2, 12] = 1 / 17
    # trans_matrix[2, 14] = 1 / 17
    # trans_matrix[2, 16] = 1 / 17
    # trans_matrix[2, 18] = 1 / 17
    # trans_matrix[2, 20] = 1 / 17
    # trans_matrix[2, 22] = 1 / 17
    # trans_matrix[2, 24] = 1 / 17
    # trans_matrix[2, 26] = 1 / 17
    # trans_matrix[2, 28] = 1 / 17
    # trans_matrix[2, 30] = 1 / 17
    # trans_matrix[2, 32] = 1 / 17
    # trans_matrix[2, 34] = 1 / 17
    # trans_matrix[3, 3] = 1 / 17
    # trans_matrix[3, 5] = 1 / 17
    # trans_matrix[3, 7] = 1 / 17
    # trans_matrix[3, 9] = 1 / 17
    # trans_matrix[3, 11] = 1 / 17
    # trans_matrix[3, 13] = 1 / 17
    # trans_matrix[3, 15] = 1 / 17
    # trans_matrix[3, 17] = 1 / 17
    # trans_matrix[3, 19] = 1 / 17
    # trans_matrix[3, 21] = 1 / 17
    # trans_matrix[3, 23] = 1 / 17
    # trans_matrix[3, 25] = 1 / 17
    # trans_matrix[3, 27] = 1 / 17
    # trans_matrix[3, 29] = 1 / 17
    # trans_matrix[3, 31] = 1 / 17
    # trans_matrix[3, 33] = 1 / 17
    # trans_matrix[4, 35] = 1
    # trans_matrix[5, 36] = 1
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz,
                                                          ...
                                                          batch_label, batch_bound, batch_type, batch_bert, bert_mask,
                                                          trans_matrix)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--train', default="./data/Address/trainn.conll")
    # parser.add_argument('--dev', default="./data/Address/devv.conll")
    # parser.add_argument('--test', default="./data/Address/testt.conll")
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/save.dset")
    parser.add_argument('--train',
                        default=r"D:\CodePractice\Python\address\data\Address\trainn.conll")
    parser.add_argument('--dev',
                        default=r"D:\CodePractice\Python\address\data\Address\devv.conll")
    parser.add_argument('--test',
                        default=r"D:\CodePractice\Python\address\data\Address\testt.conll")
    ...
    parser.add_argument('--resultfile', default="./result.txt")
    ...
    # char_emb = "./embedding/gigaword_chn.all.a2b.uni.ite50.vec"
    # bichar_emb = "./embedding/gigaword_chn.all.a2b.bi.ite50.vec"
    # gaz_file = "./embedding/ctb.50d.vec"
    char_emb = r"D:\CodePractice\Python\address\LexiconAugmentedNER-master\embedding\gigaword_chn.all.a2b.uni.ite50.vec"
    bichar_emb = r"D:\CodePractice\Python\address\LexiconAugmentedNER-master\embedding\gigaword_chn.all.a2b.bi.ite50.vec"
    gaz_file = r"D:\CodePractice\Python\address\LexiconAugmentedNER-master\embedding\ctb.50d.vec"
